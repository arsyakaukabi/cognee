...
async def cognee_network_visualization(graph_data, destination_file_path: str = None):
    ...
    logo_path = repo_root / "assets" / DEFAULT_LOGO_FILENAME
    logo_tag = ""
    if logo_path.exists():
        try:
            ...
            logo_tag = (
                ...
                + encoded_logo
                + '" style="position: fixed; bottom: 10px; right: 10px; width: 150px; height: auto; z-index: 9999;" alt="Logo" />'
            )
        except Exception as exc:  # pragma: no cover - defensive
            logger.warning("Could not embed custom logo: %s", exc)
    else:
        logger.warning("Custom logo not found at %s", logo_path)
        logo_tag = '<div style="position: fixed; bottom: 10px; right: 10px; color: #fff;">Cognee</div>'
    ...
    html_content = html_content.replace("{logo_tag}", logo_tag)
